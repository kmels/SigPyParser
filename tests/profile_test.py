...
class TestProfileTrackRecord(unittest.TestCase):
    ...
    def test_1(self):
        ...
        ###
        assert sig1['event'] == 'tp_hit'
        assert not 'secondary_scores' in sig1
        track_record_1 = track_record_of_selection([sig1])
        assert track_record_1['nwins'] == 1
        assert track_record_1['pips'] == sig1['signal']['tp_pips']
        ###
        assert sig2['event'] == 'tp_hit'
        assert 'secondary_scores' in sig2
        assert len(sig2['secondary_scores']) == 2
        assert [ss['event'] for ss in sig2['secondary_scores']] == ['tp_hit', 'sl_hit']
        track_record_2 = track_record_of_selection([sig2])
        assert track_record_2['nwins'] == round(2/3, 2)
        #pipsum = sig2['signal']['tp_pips'] + sig2['secondary_scores'][0]['signal']['tp_pips']
        #assert track_record_2['pips'] == pipsum
        pipmax = max(sig2['signal']['tp_pips'], sig2['secondary_scores'][0]['signal']['tp_pips'],)
        assert track_record_2['pips'] == round(pipmax,1)
        ###
        signals = [sig1, sig2]
        track_record = track_record_of_selection(signals)

        assert (track_record['nwins'] == 1.67)
        assert (track_record['nlosses'] == 0)

        assert track_record['roi'] == 0.79, "Unexpected roi: %s " % track_record['roi']
        pipsum = track_record_2['pips'] + track_record_1['pips']
        assert track_record['pips'] == round(pipsum,1), "Unexpected pips: %s " % track_record['pips']
    ...
    def test_2(self):
        sig1 = {'hash': 190272512845069, 'granularity': '15', 
            'event': 'tp_hit', 
                'calificador': 'SimpleFX Ltd.', 
                'account_number': 574754, 
                'pair': 'USDCAD', 'state': 'C', 
                'opened_on': {'ts': 1610623800, 'mt4': '2021.01.14 11:30'}, 
                'invalidated_on': {'ts': 1610631000, 'mt4': '2021.01.14 13:30'}, 
                'published_on': {'ts': 1610624280, 'mt4': '2021.01.14 11:38'}, 
                'last_checked': {'ts': 1610631000, 'mt4': '2021.01.14 13:30'}, 
                'last_available': {'ts': 1610747100, 'mt4': '2021.01.15 21:45'}, 
                'highest_price': 1.27641, 'lowest_price': 1.26244, 
                'highest_price_entry_pips': 87.1, 'lowest_price_entry_pips': 52.6, 
                'highest_price_date': '2021.01.15 15:45', 
                'lowest_price_date': '2021.01.14 17:45', 
                'signal': {'entry': 1.2677, 'sl': 1.26, 'tp': [1.2697, 1.2727, 1.2777], 
                'date': '2021.01.14 11:38', 'sign': 'BUY', 'pair': 'USDCAD', 'hash': 190272512845069, 
            'tp_pips': 20.0, 
            'sl_pips': 77.0}, 
            'secondary_scores': [{'hash': 190272512845069, 'granularity': '15', 
                'event': 'tp_hit', 
                    'calificador': 'SimpleFX Ltd.', 
                    'account_number': 574754, 'pair': 'USDCAD', 'state': 'C', 
                    'opened_on': {'ts': 1610623800, 'mt4': '2021.01.14 11:30'}, 
                    'invalidated_on': {'ts': 1610719200, 'mt4': '2021.01.15 14:00'}, 
                    'published_on': {'ts': 1610624280, 'mt4': '2021.01.14 11:38'}, 
                    'last_checked': {'ts': 1610719200, 'mt4': '2021.01.15 14:00'}, 
                    'last_available': {'ts': 1610747100, 'mt4': '2021.01.15 21:45'}, 
                    'highest_price': 1.27641, 'lowest_price': 1.26244, 'highest_price_entry_pips': 87.1, 
                    'lowest_price_entry_pips': 52.6, 'highest_price_date': '2021.01.15 15:45', 
                    'lowest_price_date': '2021.01.14 17:45', 'signal': {'entry': 1.2677, 'sl': 1.26, 
                    'tp': 1.2727, 'date': '2021.01.14 11:38', 'sign': 'BUY', 'pair': 'USDCAD', 'hash': 190272512845069, 
                    'tp_pips': 50.0, 'sl_pips': 77.0}},
                    {'hash': 190272512845069, 'granularity': '15', 'event': 'open', 'calificador': 'SimpleFX Ltd.', 'account_number': 574754, 'pair': 'USDCAD', 'state': 'O', 'opened_on': {'ts': 1610623800, 'mt4': '2021.01.14 11:30'}, 'invalidated_on': {'ts': 0, 'mt4': '1970.01.01 00:00'}, 'published_on': {'ts': 1610624280, 'mt4': '2021.01.14 11:38'}, 'last_checked': {'ts': 1610746200, 'mt4': '2021.01.15 21:30'}, 'last_available': {'ts': 1610747100, 'mt4': '2021.01.15 21:45'}, 'spot_price': 1.27388, 'spot_price_pips': 61.8, 'highest_price': 1.27641, 'lowest_price': 1.26244, 'highest_price_entry_pips': 87.1, 'lowest_price_entry_pips': 52.6, 'highest_price_date': '2021.01.15 15:45', 'lowest_price_date': '2021.01.14 17:45', 'signal': {'entry': 1.2677, 'sl': 1.26, 'tp': 1.2777, 'date': '2021.01.14 11:38', 'sign': 'BUY', 'pair': 'USDCAD', 'hash': 190272512845069, 'tp_pips': 100.0, 'sl_pips': 77.0}}]}
        
        track_record = track_record_of_selection([sig1])

        def tst(field, exp):
            error_msg = "Unexpected %s: %s" % (field, track_record[field])
            assert track_record[field] == exp, field
        
        tst("pips", 50.0)
        tst("nwins", 0.67)
        tst("pain", 0)
        tst("gain", 0.43)
        tst("gain_to_pain_ratio", 0)
